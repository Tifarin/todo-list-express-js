...
export const createTask = async (request, response, next) => {
    try {
        ...
        let is_completed = request.body.is_completed;
        const [result] = await TaskRepo.createData(user_id, task_name, is_completed);
...
export const updateTask = async(request, response, next) => {
    try {
        ...
        let task_name = request.body.task_name;
        let is_completed = request.body.is_completed;
        const [result] = await TaskRepo.updateData(user_id, task_name, is_completed, id);
        if(result.affectedRows > 0) {
